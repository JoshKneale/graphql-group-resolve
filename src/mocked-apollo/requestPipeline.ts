...
import { GraphQLExtension, GraphQLExtensionStack, enableGraphQLExtensions } from 'graphql-extensions';
import { DataSource } from 'apollo-datasource';
import { PersistedQueryOptions } from 'apollo-server-core/dist/index';
...
import { ApolloServerPlugin, GraphQLRequestListener, WithRequired } from 'apollo-server-plugin-base';
...
export async function processGraphQLRequestNew<TContext>(
  ...
  if (extensions && extensions.persistedQuery) {
    ...
    if (!config.persistedQueries || !config.persistedQueries.cache) {
      ...
      throw new InvalidGraphQLRequestError('Unsupported persisted query version');
    }
    ...
    if (query === undefined) {
      ...
      if (queryHash !== computedQueryHash) {
        throw new InvalidGraphQLRequestError('provided sha does not match query');
      }
      ...
      (async () => {
        return config.persistedQueries && config.persistedQueries.cache.set(`apq:${queryHash}`, query);
      })().catch(error => {
        console.warn(error);
      });
    }
  ...
  try {
    ...
    const validationDidEnd = await dispatcher.invokeDidStartHook('validationDidStart', requestContext as WithRequired<
    ...
    await dispatcher.invokeHookAsync('didResolveOperation', requestContext as WithRequired<
      typeof requestContext,
      'document' | 'operation' | 'operationName'
    >);

    const executionDidEnd = await dispatcher.invokeDidStartHook('executionDidStart', requestContext as WithRequired<
      typeof requestContext,
      'document' | 'operation' | 'operationName'
    >);
    ...
    try {
      response = (await execute(document, request.operationName, request.variables)) as GraphQLResponse;
      executionDidEnd();
  ...
  async function execute(
    ...
    try {
      /// ADDED CODE ///
      if (query && query[0] === '{') {
        return groupResolve({ queryString: query }, graphql.execute, executionArgs);
      }
      /////////////////
  ...
  async function sendResponse(response: GraphQLResponse): Promise<GraphQLResponse> {
    ...
    await dispatcher.invokeHookAsync('willSendResponse', requestContext as WithRequired<typeof requestContext, 'response'>);
    return requestContext.response!;
  }

  function initializeRequestListenerDispatcher(): Dispatcher<GraphQLRequestListener> {
